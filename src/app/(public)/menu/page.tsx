...
import { dummy_menu_items, categories } from "@/lib/data/menu";
...
const MenuPage = () => {
  ...
  const filteredItems = menuItems.filter((item) => {
    const categoryMatch =
      ...
      getCategoryName(Number(item?.category)) === selectedCategory;
  ...
  return (
    <div className="px-4 py-8 max-w-7xl mx-auto">
        ...
        <div className="flex flex-row gap-3 overflow-x-auto md:overflow-visible md:flex-wrap justify-start md:justify-center mt-2 mb-10 px-2 scrollbar-hide scroll-smooth snap-x">
          {["All", ...categories.map((cat) => cat.name)].map((cat) => (
            <button
              ...
              className={` text-black font-serif py-2 text-center px-4 rounded-full shadow-lg hover:shadow-xl hover:scale-110 transition-all duration-300 ${selectedCategory === cat ? "bg-primary text-white" : "border-gray-300 hover:bg-gray-100"}`}
            >
              {cat}
            </button>
          ))}
      ...
      <div className="grid gap-6 mt-8 sm:grid-cols-2 md:grid-cols-4">
        {filteredItems.map((item: DummyMenuItemType) => (
