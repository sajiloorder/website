...
export default function DisplayMenuItem({ item, isOpen, onClose }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 cursor-pointer " onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="bg-white rounded-lg shadow-lg p-6 pt-[100px] md:p-6 min-w-[300px] max-w-[1000px] h-auto  relative">
        <button className="absolute top-5 right-6 cursor-pointer  text-black rounded-full" onClick={onClose} aria-label="Close">
        ...
        <section>
          <div className=" p-6 grid md:grid-cols-2 gap-6 bg-white shadow-xl  rounded-xl overflow-hidden ">
            {/* image  */}
            <img src={item.image_url} alt={item.name} className="w-full h-80 object-cover rounded-md" />
            {/* content */}
            <div className=" relative flex flex-col gap-2 ">
              ...
              <Button onClick={handleAddToCart} variant="primary" size="sm" disabled={!item.is_available} className={` absolute bottom-0 right-0 mt-4 w-full py-2 cursor-pointer text-white rounded-md text-base transition ${item.is_available ? "bg-primary hover:bg-primary-dark" : "bg-gray-400 cursor-not-allowed"}`}>
