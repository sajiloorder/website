...
export default function HeroSection({ item }: { item?: DummyMenuItemType }) {
  return (
    <section className="h-[200px] sm:h-[250px] md:h-[90vh]  w-full flex flex-col md:flex-row justify-between p-4 md:p-8 bg-primary rounded-lg overflow-hidden">
      ...
      <div className="flex-1 flex flex-col justify-center items-start gap-2 px-4 md:px-8">
        <h3 className="text-base sm:text-sm md:text-2xl font-semibold text-white normal-case ">
          {item?.name}
        </h3>
        <p className="hidden sm:block text-white text-sm md:text-lg">
          {item?.description}
        </p>
