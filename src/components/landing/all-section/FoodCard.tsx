...
export default function FoodCard({ name, image, price, rating, tag }: { name: string; image: string; price: number; rating: number; tag?: string }) {
  return (
    <div className="relative bg-white rounded-md overflow-hidden shadow-md hover:shadow-lg transition duration-200 ">
      {/* Tag */}
      {tag && <span className="absolute top-2 left-2 bg-red-500 text-white text-xs px-2 py-1 rounded-md z-10">{tag}</span>}
